...
class SyncClient {
   ...
   * @param {Number} [options.estimationStability=180e-6] adaptation to the
   *   estimated server time is limited by this value. 80e-6 (80 parts per
   *   million, PPM) is quite stable, and corresponds to the stability of a
   *   conventional clock. 160e-6 is moderately adaptive, and corresponds to the
   *   relative stability of 2 clocks; 500e-6 is quite adaptive, and corresponds
   *   to an old mechanical clock; 10000e-6 compensates 10 milliseconds in 1 second.
   ...
   *   frequency
   * @param {Number} [options.longTermDataDuration=900] estimate synchronisation
   *    over this duration, in seconds, approximately
   */
  constructor(getTimeFunction, options = {}) {
    this.estimationStability = options.estimationStability || 160;
    ...
    // Gradually apply offset when the references change, to stabilise
    // estimations

    // S(t) = T(t) + (t - tss) * Rs, with t in [tss, tse]
    // S(t) = T(t), with t not in [tss, tse]
    this.clientTimeSmoothStart = 0; // tss
    this.clientTimeSmoothEnd = 0; // tss
    this.clientTimeSmoothRatio = 1; // Rs

    // s(T) = t(T) + (T - Tss) / Rs, with T in [Tss, Tse]
    // s(T) = t(T), with T not in [Tss, Tse]
    this.serverTimeSmoothStart = 0; // Tss
    this.serverTimeSmoothEnd = 0; // Tse
